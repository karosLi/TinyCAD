...
int COutputWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (!m_wndCmd.Create(dwStyle, rectDummy, &m_wndTabs, 2))
	{
		TRACE0("未能创建命令输入窗口!\n");
		return -1;      // 未能创建
	}

	m_wndCmd.SetFont(&m_Font);

	CString strTabName;
	BOOL bNameValid;

	// 将窗口附加到选项卡:
	bNameValid = strTabName.LoadString(IDS_WND_CMDINPUT);
	ASSERT(bNameValid);
	m_wndTabs.AddTab(&m_wndCmd, strTabName, (UINT)0);

	dwStyle = LBS_NOINTEGRALHEIGHT | WS_CHILD | WS_VISIBLE | WS_HSCROLL | WS_VSCROLL;

	if (!m_wndErr.Create(dwStyle, rectDummy, &m_wndTabs, 3))
	{
		TRACE0("未能创建错误信息窗口!\n");
		return -1;      // 未能创建
	}

	m_wndErr.SetFont(&m_Font);

	// 将窗口附加到选项卡:
	bNameValid = strTabName.LoadString(IDS_WND_ERROUTPUT);
	ASSERT(bNameValid);
	m_wndTabs.AddTab(&m_wndErr, strTabName, (UINT)1);
